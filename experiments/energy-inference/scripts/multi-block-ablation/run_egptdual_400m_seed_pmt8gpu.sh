#!/bin/bash
# ============================================================================
# 400M EGPT-Dual SEED run — 8 H100 GPUs (single node), PREEMPTABLE / grp_preemptable.
...
#
#   Usage: run_egptdual_400m_seed_pmt8gpu.sh <config.yml> <save_path> <job_name>
#
...
# save_interval 2000 => can eval iteration-reduction at ~15k (~31B) / 30k (~63B).
...
# for uniformity (fans to the one allocated host). Self-resubmits on preemption
# via latest_checkpointed_iteration.json; unshards + evals after the final step.
# ============================================================================
set -euo pipefail
...
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_egptdual_400m_seed_pmt8gpu.sh
...
BSUB
echo "Submitted ${JOB_NAME} (8 GPU preemptable/grp_preemptable, single node): ${CONFIG}"
