#!/bin/bash
# 60k-step fresh run of h1 PSD-anti d=1280 on 8 H100 GPUs (single-node),
# normal queue / grp_ebm.
#
# Effective batch matches the original 4-GPU 30k run:
#   8 GPU x mb=4 x ga=2 x seq=4096 = 262144 tokens/step (= original 4x4x4x4096)
# So the LR schedule (warmup 2000 → cosine decay 58000) lines up with the
# original at every step ratio. Total: 60k steps x 262144 = 15.7B tokens
# (= 56 tok/param at 279M; vs the original 30k = 7.86B = 28 tok/param).
#
# Self-resubmits via latest_checkpointed_iteration.json. After completion,
# unshards and submits LM-harness + BBH eval.
set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=${REPO}/configs/multi_block_ablation/math_psda_h1_d1280_60k.yml
SAVE_PATH=${REPO}/experiments/energy-inference/results/multi-block-ablation/math_psda_h1_d1280_60k
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_math_psda_h1_d1280_60k.sh
JOB_NAME=math_psda_h1_d1280_60k
mkdir -p "${HOME}/bsub_logs"
bsub \
    -q normal -G grp_ebm -J ${JOB_NAME} \
    -gpu "num=8/task:mode=exclusive_process" -n 1 -M 96G -W 12:00 \
