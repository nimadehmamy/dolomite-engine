#!/bin/bash
# Hopfield-MEAN + r128 + start_layer=8 + SOFT-BAND attn-balance aux, 16 GPU big run.
# 8G+1Ex6 d=1536 with Hopfield_Energy_MLP (single weight W, ||gelu(Wh)||^2 — bounded
# below by 0). Soft-band register balance (parity floor rho_lo=1 + soft ceiling
# rho_hi=4) instead of the strongbal ceiling hinge. Iso-data with Hopfield-MEAN base.
# Trap-protected auto-resubmit; auto-eval at completion.
set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
TAG=math_fet_hopfield_mean_r128_start8_softband_8gpt_1egpt6x_d1536_int8k_lra32_itd3_lr1p5e3_33b_8gpu1node
...
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_math_fet_hopfield_r128_start8_softband_d1536_int8k_lr1p5e3_33b_8gpu1node.sh
JOB_NAME=math_fet_hop_r128s8_softband_8gpu1node
...
post_exit() {
    ...
    if [ "\${LATEST_ITER}" -lt "\${NUM_TRAINING_STEPS}" ]; then
        ...
        bash ${REPO}/experiments/eval_scripts/submit_bbh_eval.sh "\${UNSHARDED}" "eval_\${JOB_NAME}_bbh"
        bsub -q normal -G grp_ebm -J "verif_\${JOB_NAME}" \
            -gpu "num=1/task:mode=exclusive_process" -n 1 -M 48G -W 04:00 \
            -o "\${HOME}/bsub_logs/verif_\${JOB_NAME}_%J.stdout" \
            -e "\${HOME}/bsub_logs/verif_\${JOB_NAME}_%J.stderr" \
            /bin/bash -c "unset TMPDIR TEMP TMP; source /proj/dmfexp/nima/Code/nanoGPT-og/.venv/bin/activate; export PYTHONPATH=${REPO}:\\\${PYTHONPATH:-}; cd /proj/dmfexp/nima/Code/GPT-experiments; python projects/EGPT-RL/scripts/multitask_action_scoring_explicit_ckpt_20260628.py --ckpt-path \${UNSHARDED} --model-tag hop_r128s8_softband_8gpu --out-dir /proj/dmfexp/nima/Code/GPT-experiments/projects/EGPT-RL/results/verifier_contrast_sweep_20260628"
    fi
}
trap post_exit EXIT
...
BSUB
echo "Submitted ${JOB_NAME} (8 GPU single-node Hopfield-FET soft-band (iso-batch ga8), normal grp_ebm, 33.5B tokens)"
