#!/bin/bash
# FET-1.5e-3 baseline: same arch as the original FET (8G+1Ex6 d=1536 LRA-32 itd3
# psd_anti energy_full_per_token_grad), but trained from scratch with the same
# batching/schedule as math_recgpt (8 GPU mb=4 ga=8 = 1.05M tok/step, 32k steps,
# peak 1.5e-3 cosine). Direct head-to-head vs math_recgpt at iso-tokens.
# 8 GPU single-node on normal queue grp_ebm; trap-protected auto-resubmit; auto-eval.
set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=${REPO}/configs/multi_block_ablation/math_fet_8gpt_1egpt6x_d1536_lra32_itd3_lr1p5e3_33b_8gpu.yml
SAVE_PATH=${REPO}/experiments/energy-inference/results/multi-block-ablation/math_fet_8gpt_1egpt6x_d1536_lra32_itd3_lr1p5e3_33b_8gpu
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_math_fet_8gpt_1egpt6x_d1536_lra32_itd3_lr1p5e3_33b_8gpu.sh
JOB_NAME=math_fet_lr1p5e3_33b_8g
...
echo "[head] launching pretrain.sh on 1 node (8 GPU single-node)"
bash ${REPO}/scripts/common/pretrain.sh "\${RUN_CONFIG}" || echo "[main] pretrain returned \$? — trap will still run"
BSUB
echo "Submitted ${JOB_NAME} (8 GPU single-node FET 8G+1Ex6 d=1536 LRA-32 itd3 lr=1.5e-3, normal grp_ebm, ≥33B tokens)"
