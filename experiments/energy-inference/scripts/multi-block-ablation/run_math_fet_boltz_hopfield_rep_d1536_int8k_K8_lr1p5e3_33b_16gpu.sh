#!/bin/bash
# Boltzmann-MoE Hopfield + REPULSION 16 GPU big run. 8G+1Ex6 d=1536 with
# BoltzmannMoE_Hopfield_Energy_MLP (K=8 experts each with single-W Hopfield
# form, ||gelu(W_k h)||^2/expert_I; Boltzmann routing softmax(-E_k); total
# energy E_total = -LSE_k(-E_k) bounded both directions).  Iso-param /
# iso-data with the Hopfield-MEAN baseline (job 1751549).
# Trap-protected auto-resubmit; auto-eval at completion.
set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
TAG=math_fet_boltz_hopfield_rep_8gpt_1egpt6x_d1536_int8k_K8_lra32_itd3_lr1p5e3_33b_16gpu
...
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_math_fet_boltz_hopfield_rep_d1536_int8k_K8_lr1p5e3_33b_16gpu.sh
JOB_NAME=math_fet_bzhop_rep_33b_16g
...
BSUB
echo "Submitted ${JOB_NAME} (16 GPU multi-node Boltzmann-MoE Hopfield FET K=8, normal grp_ebm, 33.5B tokens)"
